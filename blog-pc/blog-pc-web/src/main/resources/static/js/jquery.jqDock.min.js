        ...
        var jqDock = function () {
            return {
                ...
                initDock: function (id) {
                    var ME = this, Dock = this.docks[id], op = Dock.Opts, off = 0, AI = $('a, img', Dock.Menu), i = 0,
                        j, el, wh, acc, upad,
                        opPre95 = ($.support.opera && (1 * ($.support.version.match(/^(\d+\.\d+)/) || [0, 0])[1]) < 9.5);
                    ...
                    var wrap = [this.vanillaDiv, '<div class="jqDock" style="position:absolute;top:0px;left:0px;padding:0px;', 'margin:0px;overflow:visible;height:', Dock.height, 'px;width:', Dock.width, 'px;"></div></div>'].join('');
                ...
                deltaXY: function (id) {
                    ...
                    if (Dock.Current !== false) {
                        var op = Dock.Opts, el = Dock.Elem[Dock.Current], p = el.Pad[op.vh.lead] + el.Pad[op.vh.trail],
                            off = el.Img.offset();
                ...
                doLabel: function (id, off) {
                    ...
                    if (Dock.Opts.labels && Dock.Current !== false) {
                        var el = Dock.Elem[Dock.Current], L = el.Label, op = Dock.Opts,
                        ...
                        switch (what) {
                            ...
                            default:
                                var doff = Dock.Yard.offset(),
                                    css = {top: off.top - doff.top, left: off.left - doff.left},
                ...
                mouseHandler: function (e) {
                    var r = null, t = e.target.className.match(/jqDockMouse(\d+)_(\d+)/),
                ...
                changeSize: function (id, idx, dim, force) {
                    ...
                    if (force || el.Actual != dim) {
                        ...
                        var css = this.keepProportion(el, dim, op.orient), diff = op.size - css[op.vh.inv.wh],
                            m = 'margin', z = op.vh.inv;
                        switch (op.align) {
                            case'bottom':
                            case'right':
                                css[m + z.lead] = diff;
        ...
        $.fn.jqDock = function (opts) {
            return this.filter(function () {
                ...
                $('img', this).each(function (n) {
                    var me = $(this), s0 = me.attr('src'),
                        s1 = (op.source ? op.source.call(me[0], n) : false) || jqDock.altImage.call(this) || s0,
                        tx = op.labels ? me.attr('title') || me.parent('a').attr('title') || '' : '';
                    Dock.Elem[n] = {
                        Img: me,
                        Source: [s0, s1, !(s0 == s1)],
